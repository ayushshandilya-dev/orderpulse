...
# Example:
#   us-docker.pkg.dev/my-project/orderpulse
#
# Images pushed by CI:
#   us-docker.pkg.dev/my-project/orderpulse/control-plane:$SHA
#   us-docker.pkg.dev/my-project/orderpulse/worker:$SHA
#   us-docker.pkg.dev/my-project/orderpulse/sandbox-go:$SHA
#   us-docker.pkg.dev/my-project/orderpulse/sandbox-rust:$SHA
#   us-docker.pkg.dev/my-project/orderpulse/sandbox-cpp:$SHA
#   us-docker.pkg.dev/my-project/orderpulse/sandbox-python:$SHA
#   us-docker.pkg.dev/my-project/orderpulse/sandbox-binary:$SHA

# ── Artifact Registry repository ─────────────────────────────────────────────

resource "google_artifact_registry_repository" "orderpulse" {
  location      = var.registry_location
  repository_id = var.registry_name
  format        = "DOCKER"
  project       = var.project_id
  description   = "OrderPulse container images (control-plane, workers, sandboxes)"
...
resource "google_artifact_registry_repository_iam_member" "node_pull" {
  location   = google_artifact_registry_repository.orderpulse.location
  repository = google_artifact_registry_repository.orderpulse.name
  project    = var.project_id
...
resource "google_service_account" "ci_push" {
  ...
  display_name = "OrderPulse CI image push service account"
  project      = var.project_id
}
