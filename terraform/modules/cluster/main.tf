...
# ── VPC & Subnets ─────────────────────────────────────────────────────────────
# A dedicated VPC isolates OrderPulse traffic from any default VPC resources.
# Using a custom VPC also gives us full control over subnet CIDRs.

resource "google_compute_network" "orderpulse" {
...
resource "google_compute_subnetwork" "nodes" {
  ...
  network = google_compute_network.orderpulse.id
  region  = var.region
  project = var.project_id
...
resource "google_compute_router" "orderpulse" {
...
resource "google_compute_router_nat" "orderpulse" {
  name                               = "${var.cluster_name}-nat"
  router                             = google_compute_router.orderpulse.name
  region                             = var.region
  project                            = var.project_id
...
resource "google_service_account" "gke_nodes" {
  ...
  display_name = "OrderPulse GKE node service account"
  project      = var.project_id
}
...
resource "google_iam_workload_identity_pool" "github" {
  ...
  display_name              = "GitHub Actions for OrderPulse"
  project                   = var.project_id
...
resource "google_container_cluster" "orderpulse" {
