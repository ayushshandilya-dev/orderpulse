...
output "cluster_name" {
  ...
  value       = google_container_cluster.orderpulse.name
}

output "endpoint" {
  description = "Internal IP address of the GKE API server."
  value       = google_container_cluster.orderpulse.endpoint
  sensitive   = true
}

output "ca_certificate" {
  description = "Base64-encoded cluster CA certificate."
  value       = google_container_cluster.orderpulse.master_auth[0].cluster_ca_certificate
  sensitive   = true
}
...
output "network_id" {
  ...
  value       = google_compute_network.orderpulse.id
}
