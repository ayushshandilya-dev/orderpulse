...
variable "control_plane_machine_type" {
  description = <<-EOT
    ...
    Hosts: OrderPulse control plane, metrics-consumer, Prometheus, Grafana,
...
variable "services_ipv4_cidr" {
  description = <<-EOT
    ...
    /20 gives 4 096 service IPs — far more than OrderPulse needs.
  EOT
  type        = string
  default     = "10.2.0.0/20"
}
