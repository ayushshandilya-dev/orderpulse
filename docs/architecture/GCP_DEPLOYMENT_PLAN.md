# OrderPulse — GCP Cloud Deployment Implementation Plan
...
export CLUSTER_NAME="orderpulse"               # matches variables.tf default
export REGISTRY_LOCATION="us"                  # matches variables.tf default
export REGISTRY_NAME="orderpulse"              # matches variables.tf default
export GITHUB_REPO="your-org/orderpulse"       # owner/repo format, no https://
...
gsutil mb -p $PROJECT_ID -l $REGION gs://${PROJECT_ID}-orderpulse-tfstate
gsutil versioning set on gs://${PROJECT_ID}-orderpulse-tfstate
...
cd terraform
terraform init \
  -backend-config="bucket=${PROJECT_ID}-orderpulse-tfstate" \
  -backend-config="prefix=orderpulse/dev" \
  -reconfigure
cd ..
```
...
The control-plane `deployment.yaml` uses `envFrom: configMapRef: orderpulse-config`.
...
```bash
kubectl create secret generic orderpulse-db-secrets \
...
`worker/deployment.yaml` declares `serviceAccountName: orderpulse-worker`.
Without this ServiceAccount the worker pods fail with
`serviceaccounts "orderpulse-worker" not found`.
...
kind: ServiceAccount
metadata:
  name: orderpulse-worker
  namespace: orderpulse
  labels:
    app.kubernetes.io/part-of: orderpulse
```
...
Both Deployments use `envFrom: configMapRef: orderpulse-config`. The env var
...
kind: ConfigMap
metadata:
  name: orderpulse-config
  namespace: orderpulse
data:
  LISTEN_ADDR: ":8080"
  SUBMISSION_DIR: "/data/submissions"
  DISTRIBUTED_MODE: "true"
  REDPANDA_BROKERS: "redpanda-svc.orderpulse.svc.cluster.local:9092"
  POSTGRES_DSN: "postgres://orderpulse:$(POSTGRES_PASSWORD)@postgres-svc.orderpulse.svc.cluster.local:5432/orderpulse"
  TIMESCALE_DSN: "postgres://orderpulse:$(TIMESCALE_PASSWORD)@timescaledb-svc.orderpulse.svc.cluster.local:5432/orderpulse"
  ORCHESTRATOR_URL: "http://control-plane-svc.orderpulse.svc.cluster.local:8080"
...
`redpanda-svc.orderpulse.svc.cluster.local:9092` — the exact address in the
ConfigMap above.
...
kubectl wait --for=condition=ready pod \
  -l app=redpanda -n orderpulse --timeout=120s
kubectl wait --for=condition=ready pod \
  -l app=postgres -n orderpulse --timeout=120s
kubectl wait --for=condition=ready pod \
  -l app=timescaledb -n orderpulse --timeout=120s
```
...
spec:
  ...
  triggers:
    - type: kafka
      metadata:
        bootstrapServers: redpanda-svc.orderpulse.svc.cluster.local:9092
        consumerGroup: orderpulse-worker
        topic: orderpulse-jobs
...
kubectl get scaledobject -n orderpulse
...
```yaml
# Default deny all ingress and egress in the orderpulse namespace.
...
kubectl rollout status deployment/control-plane -n orderpulse --timeout=120s
...
kubectl get pods -n orderpulse -l app.kubernetes.io/name=control-plane
# Expected: 1/1 Running

# Health check through port-forward
kubectl port-forward svc/control-plane-svc 8080:8080 -n orderpulse &
sleep 2
...
export ORDERPULSE_URL="http://localhost:8080"
...
kubectl get hpa -n orderpulse -w
```
...
kubectl rollout restart deployment/control-plane -n orderpulse
```
...
```bash
kubectl rollout undo deployment/control-plane -n orderpulse
kubectl rollout undo deployment/worker -n orderpulse
kubectl rollout status deployment/control-plane -n orderpulse
```
...
kubectl delete namespace orderpulse
...
gsutil rm -r gs://${PROJECT_ID}-orderpulse-tfstate
```
