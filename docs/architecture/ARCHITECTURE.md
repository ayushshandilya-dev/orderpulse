# OrderPulse — Architecture & Engineering Decision Record

> **Audience:** Technical engineers, system architects, and AI agents working on or evaluating this codebase.
> This document is the definitive record of how OrderPulse was designed, what tradeoffs were made at each phase, and why the system looks the way it does today. Read this before touching any code.

---

## Table of Contents

1. [What OrderPulse Is](#1-what-orderpulse-is)
...
## 1. What OrderPulse Is

OrderPulse is a **distributed benchmarking and contest platform for Central Limit Order Book (CLOB) matching engines**, built for the IICPC Summer Hackathon 2026 (May 9 – June 10).

A contestant uploads a matching engine written in Go, Rust, C++, Python, or as a precompiled binary. OrderPulse:
...
**What is being evaluated:** OrderPulse evaluates CLOB matching engine correctness and performance under load. It does not evaluate trading strategies, portfolio managers, or execution algorithms. The engine must accept orders, maintain a price-time priority orderbook, and return fills correctly — at increasing concurrency.
...
- **KEDA ScaledObject** — watches the `orderpulse-workers` Redpanda consumer group lag; scales workers from 1 to 10 replicas; scales to zero when lag is zero (queue drained)
...
**Cause:** The control plane joined the `orderpulse-workers` consumer group. Redpanda distributed partitions equally across all group members. Since the control plane does not process jobs, submissions assigned to its partitions were never dequeued.
...
**Transport:** HTTP/1.1 on `$ORDERPULSE_LISTEN_PORT` (default `7878`). REST/JSON or WebSocket (configurable at submission time).
...
**Packaging:** A `.tar.gz` archive containing source code (the sandbox will compile it) or a pre-compiled binary named `engine`. The sandbox entrypoint sets `$ORDERPULSE_LISTEN_PORT` before invoking the binary.

---

*OrderPulse — IICPC Summer Hackathon 2026.*
