...
#   ORDERPULSE_SUBMISSION_ID, ORDERPULSE_TEAM, ORDERPULSE_LANGUAGE, ORDERPULSE_PROTOCOL, ORDERPULSE_LISTEN_PORT

set -euo pipefail

log() { echo "[orderpulse] $*" >&2; }

log "Sandbox starting | submission=${ORDERPULSE_SUBMISSION_ID} team=${ORDERPULSE_TEAM} lang=${ORDERPULSE_LANGUAGE} port=${ORDERPULSE_LISTEN_PORT:-7878}"
...
binary)
    ...
    ;;
*)
    log "ERROR: unknown language '$ORDERPULSE_LANGUAGE'"
    exit 1
    ;;
esac
...
exec su -s /bin/bash runner -c "
    export ORDERPULSE_SUBMISSION_ID='${ORDERPULSE_SUBMISSION_ID}'
    export ORDERPULSE_TEAM='${ORDERPULSE_TEAM}'
    export ORDERPULSE_LANGUAGE='${ORDERPULSE_LANGUAGE}'
    export ORDERPULSE_PROTOCOL='${ORDERPULSE_PROTOCOL}'
    export ORDERPULSE_LISTEN_PORT='${ORDERPULSE_LISTEN_PORT:-7878}'
    export PATH=/usr/local/go/bin:\$PATH
    export GOPATH=/home/runner/go
    export GOCACHE=/home/runner/.cache/go-build
