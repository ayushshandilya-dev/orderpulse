...
#
# Phase 2 end-to-end smoke test for OrderPulse.
...
# ── config ────────────────────────────────────────────────────────────────────
CONTROL_PLANE="${ORDERPULSE_URL:-http://localhost:8080}"
...
TIMESCALE_DSN="${TIMESCALE_DSN:-postgres://orderpulse:orderpulse_dev@localhost:5432/orderpulse}"
...
echo ""
echo "╔═══════════════════════════════════════════════════════╗"
echo "║       OrderPulse — Phase 2 Smoke Test                ║"
echo "╚═══════════════════════════════════════════════════════╝"
echo ""
...
for job in "orderpulse-server" "cadvisor" "node-exporter" "redpanda"; do
...
# Verify OrderPulse custom metrics exist (control plane must be up)
NB_METRIC=$(get "$PROMETHEUS/api/v1/query?query=orderpulse_http_requests_total")
...
if [ "${NB_RESULT:-0}" -gt 0 ]; then
    pass "orderpulse_http_requests_total metric present"
else
    # May be 0 if no requests yet — just warn, not fail
    warn "orderpulse_http_requests_total not yet scraped — make a request to $CONTROL_PLANE/health first"
fi
...
for uid in "orderpulse-admin" "orderpulse-contestant"; do
...
if echo "$METRICS_BODY" | grep -q "orderpulse_"; then
    pass "/metrics endpoint serves OrderPulse custom metrics"
else
    fail "/metrics endpoint at $CONTROL_PLANE/metrics not serving orderpulse_ metrics"
fi
