...
#     Requires: kubectl, a running cluster with KEDA + OrderPulse deployed.
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#
...
CONTROL_PLANE_URL="${ORDERPULSE_URL:-http://localhost:8080}"
...
if [[ "$MODE" == "--dry-run" ]]; then
  ...
  check_field "consumerGroup: orderpulse-workers"  "consumerGroup: orderpulse-workers"
...
if [[ $SUBMITTED -gt 0 ]]; then
  ...
  info "If running in-cluster, set ORDERPULSE_URL to the cluster-internal URL."
fi
...
while [[ $(date +%s) -lt $DRAIN_DEADLINE ]]; do
  DEPTH=$(curl -s "$CONTROL_PLANE_URL/metrics" 2>/dev/null \
    | grep "^orderpulse_queue_depth " | awk '{print $2}' || echo "")
