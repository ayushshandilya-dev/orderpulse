...
#     $ORDERPULSE_URL (default: http://localhost:8080).
#
...
ORDERPULSE_URL="${ORDERPULSE_URL:-http://localhost:8080}"
...
# Path to a Linux amd64 binary that speaks the OrderPulse REST order protocol.
# If not set the script builds one from a minimal embedded Go source.
ENGINE_BINARY="${ENGINE_BINARY:-}"

echo ""
echo -e "${YELLOW}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${YELLOW}║   OrderPulse — Phase 5 Integration Smoke Test           ║${NC}"
...
if [[ "$MODE" == "--dry-run" ]]; then
  ...
  if go test ./... -race -timeout 90s -count=1 -coverprofile=/tmp/orderpulse_coverage.out \
       -covermode=atomic 2>/dev/null; then
    COVERAGE=$(go tool cover -func=/tmp/orderpulse_coverage.out 2>/dev/null | tail -1 | awk '{print $3}')
...
api() {
  ...
  shift 2
  curl -sf -X "$method" "${ORDERPULSE_URL}${path}" \
...
api_noauth() {
  local method="$1" path="$2"
  shift 2
  curl -s -X "$method" "${ORDERPULSE_URL}${path}" \
    -H "Content-Type: application/json" \
    "$@" 2>/dev/null
}
...
if curl -sf "${ORDERPULSE_URL}/health" 2>/dev/null | jq -e '.status == "ok"' >/dev/null 2>&1; then
  pass "Control plane healthy at ${ORDERPULSE_URL}"
else
  fail "Control plane not responding at ${ORDERPULSE_URL}"
  echo -e "${RED}  Start the stack first: docker compose up --build -d${NC}"
  exit 1
fi

SSE_OUTPUT_FILE=$(mktemp /tmp/orderpulse-sse-XXXXXX.txt)
SSE_PID=""
...
curl -sN "${ORDERPULSE_URL}/api/v1/leaderboard/stream" > "$SSE_OUTPUT_FILE" &
SSE_PID=$!
sleep 2
...
RATE_LIMIT_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -X POST "${ORDERPULSE_URL}/api/v1/submissions/${SUB_ID}/validate" 2>/dev/null || echo "000")
...
UNAUTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -X POST "${ORDERPULSE_URL}/api/v1/admin/contests" \
  -H "Content-Type: application/json" \
...
HEALTH=$(curl -sf "${ORDERPULSE_URL}/health" 2>/dev/null || echo "")
...
SUBMISSIONS_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  "${ORDERPULSE_URL}/api/v1/submissions" 2>/dev/null || echo "000")
...
LEADERBOARD_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  "${ORDERPULSE_URL}/api/v1/leaderboard" 2>/dev/null || echo "000")
