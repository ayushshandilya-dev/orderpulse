...
echo ""
echo "╔═══════════════════════════════════════════════════════╗"
echo "║   OrderPulse — Phase 3 Stage 3.1 Smoke Test          ║"
echo "╚═══════════════════════════════════════════════════════╝"
...
HTTP_CODE=$(curl -s -o /tmp/orderpulse_submit_resp.json -w "%{http_code}" \
...
if [ "$HTTP_CODE" = "201" ]; then
    ...
    SUB_ID=$(grep -o '"id":"[^"]*"' /tmp/orderpulse_submit_resp.json 2>/dev/null \
             | head -1 | cut -d'"' -f4 || echo "unknown")
    info "Submission ID: $SUB_ID"
else
    fail "POST /api/v1/submissions returned $HTTP_CODE (expected 201)"
    info "Response: $(cat /tmp/orderpulse_submit_resp.json 2>/dev/null || echo '(empty)')"
fi
