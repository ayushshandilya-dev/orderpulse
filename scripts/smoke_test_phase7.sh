...
echo ""
echo -e "${YELLOW}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${YELLOW}║   OrderPulse — Phase 7 Pre-flight Gate Smoke Test       ║${NC}"
...
if [[ "$MODE" == "--dry-run" ]]; then
  ...
  if go test ./... -race -timeout 120s -count=1 \
       -coverprofile=/tmp/orderpulse_phase7_cov.out \
       -covermode=atomic 2>/dev/null; then
    COV=$(go tool cover -func=/tmp/orderpulse_phase7_cov.out 2>/dev/null \
          | tail -1 | awk '{print $3}')
...
submit_engine() {
  ...
  curl -sf -X POST "${ORDERPULSE_URL}/api/v1/submissions" \
    -F "team_name=${team}" \
    -F "language=binary" \
    -F "protocol=rest" \
    -F "archive=@${archive}" \
...
if curl -sf "${ORDERPULSE_URL}/health" 2>/dev/null \
...
VALIDATE_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
  -X POST "${ORDERPULSE_URL}/api/v1/submissions/${BROKEN_ID}/validate" 2>/dev/null || echo "000")
...
VALIDATE_STATUS_B=$(curl -s -o /dev/null -w "%{http_code}" \
  -X POST "${ORDERPULSE_URL}/api/v1/submissions/${CORRECT_ID}/validate" 2>/dev/null || echo "000")
