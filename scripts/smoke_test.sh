...
# End-to-end smoke test against a running OrderPulse control plane.
#
# Usage:
#   bash scripts/smoke_test.sh
#   ORDERPULSE_URL=http://localhost:8080 bash scripts/smoke_test.sh

BASE="${ORDERPULSE_URL:-http://localhost:8080}"
PASS=0
FAIL=0
...
echo ""
echo "═══════════════════════════════════════"
echo " OrderPulse Smoke Test → $BASE"
echo "═══════════════════════════════════════"
...
TMPDIR_SMOKE="${TEMP:-/tmp}/orderpulse-smoke-$$"
...
func main() {
	port := os.Getenv("ORDERPULSE_LISTEN_PORT")
	if port == "" {
		port = "7878"
	}
