// Package config provides configuration loading for OrderPulse.
package config
...
// Config holds all runtime configuration for the OrderPulse control plane.
type Config struct {
	ListenAddr string
	...
	// Example: "postgres://orderpulse:secret@localhost:5432/orderpulse"
...
func Load() *Config {
	return &Config{
		...
		ImageGo:     getEnv("SANDBOX_IMAGE_GO", "orderpulse-sandbox-go:latest"),
		ImageRust:   getEnv("SANDBOX_IMAGE_RUST", "orderpulse-sandbox-rust:latest"),
		ImageCpp:    getEnv("SANDBOX_IMAGE_CPP", "orderpulse-sandbox-cpp:latest"),
		ImagePython: getEnv("SANDBOX_IMAGE_PYTHON", "orderpulse-sandbox-python:latest"),
		ImageBinary: getEnv("SANDBOX_IMAGE_BINARY", "orderpulse-sandbox-binary:latest"),
