...
func TestHandler_ServesMetrics(t *testing.T) {
	...
	if !strings.Contains(bodyStr, "orderpulse_http_requests_total") {
		t.Errorf("metrics output missing orderpulse_http_requests_total\n%s", bodyStr[:min(500, len(bodyStr))])
	}
}
...
func TestRecordHTTPRequest(t *testing.T) {
	...
	body := scrape(t, reg)
	assertContains(t, body, `orderpulse_http_requests_total{method="GET",path="/api/v1/submissions",status_code="200"} 2`)
	assertContains(t, body, `orderpulse_http_requests_total{method="POST",path="/api/v1/submissions",status_code="201"} 1`)
}
...
func TestRecordSubmission(t *testing.T) {
	...
	body := scrape(t, reg)
	assertContains(t, body, `orderpulse_submission_total{language="go",status="accepted"} 2`)
	assertContains(t, body, `orderpulse_submission_total{language="rust",status="rejected"} 1`)
}
...
func TestSandboxStartedAndStopped(t *testing.T) {
	...
	body := scrape(t, reg)
	assertContains(t, body, `orderpulse_sandbox_active{language="go"} 1`)
	assertContains(t, body, `orderpulse_sandbox_exits_total{language="go",reason="stopped"} 1`)
}
...
func TestRecordTelemetryEvent(t *testing.T) {
	...
	body := scrape(t, reg)
	assertContains(t, body, `orderpulse_telemetry_events_total{kind="order_ack"} 2`)
	assertContains(t, body, `orderpulse_telemetry_events_total{kind="fill"} 1`)
}
...
func TestRecordTelemetryEmitError(t *testing.T) {
	...
	body := scrape(t, reg)
	assertContains(t, body, `orderpulse_telemetry_emit_errors_total 2`)
}
...
func TestRecordConsumerWindow(t *testing.T) {
	...
	body := scrape(t, reg)
	assertContains(t, body, `orderpulse_consumer_windows_total{submission_id="sub-abc"} 2`)
	assertContains(t, body, `orderpulse_consumer_windows_total{submission_id="sub-xyz"} 1`)
}

// ── QueueDepth gauge tests ────────────────────────────────────────────────────

// TestSetQueueDepth_InitiallyZero verifies that a freshly created registry
// exposes orderpulse_queue_depth as 0 before SetQueueDepth is called.
func TestSetQueueDepth_InitiallyZero(t *testing.T) {
	t.Parallel()
	reg := metrics.New()
	body := scrape(t, reg)
	assertContains(t, body, "orderpulse_queue_depth 0")
}
...
func TestSetQueueDepth_UpdatesGauge(t *testing.T) {
	...
	reg.SetQueueDepth(7)
	assertContains(t, scrape(t, reg), "orderpulse_queue_depth 7")

	// Overwrite: gauge should reflect the latest call, not accumulate.
	reg.SetQueueDepth(3)
	assertContains(t, scrape(t, reg), "orderpulse_queue_depth 3")
}
...
func TestSetQueueDepth_Zero(t *testing.T) {
	...
	reg.SetQueueDepth(0)
	assertContains(t, scrape(t, reg), "orderpulse_queue_depth 0")
}
...
func TestSetQueueDepth_MetricNamePresent(t *testing.T) {
	...
	if !strings.Contains(body, "orderpulse_queue_depth") {
		t.Errorf("metrics output missing orderpulse_queue_depth gauge\n%s",
...
func TestSetQueueDepth_IsolatedFromOtherRegistries(t *testing.T) {
	...
	assertContains(t, body2, "orderpulse_queue_depth 0")
}
...
func TestIsolatedRegistries(t *testing.T) {
	...
	if !strings.Contains(body1, `orderpulse_submission_total{language="go",status="accepted"} 1`) {
		t.Error("reg1 missing expected metric")
	}
	if strings.Contains(body2, `orderpulse_submission_total{language="go",status="accepted"} 1`) {
