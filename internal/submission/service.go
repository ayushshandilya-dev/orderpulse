...
func (s *Service) deployAsync(ctx context.Context, sub *models.Submission) {
	if s.docker == nil {
		slog.Error("sandbox deploy failed: docker manager not available (distributed mode?)", "id", sub.ID)
		s.setStatus(sub, models.StatusFailed, "deploy error: docker manager not available")
		return
	}
...
func (s *Service) StopContainer(ctx context.Context, id string) error {
	if s.docker == nil {
		return fmt.Errorf("cannot stop container for %s: docker manager not available (distributed mode)", id)
	}
