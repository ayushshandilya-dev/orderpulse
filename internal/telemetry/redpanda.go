...
// Bootstrap creates all OrderPulse topics if they don't already exist.
...
func (r *RedpandaEmitter) BatchEmit(ctx context.Context, events []Event) error {
	...
	for i := range events {
		...
		records = append(records, trackedRecord{
			record: &kgo.Record{
				Topic: TopicForKind(e.Kind),
				Key:   []byte(e.SubmissionID),
				Value: payload,
	...
	if len(records) > 0 {
		for _, tr := range records {
			r.client.Produce(ctx, tr.record, nil)
			if r.metrics != nil {
				r.metrics.RecordTelemetryEvent(string(tr.kind))
			}
		}
	}
