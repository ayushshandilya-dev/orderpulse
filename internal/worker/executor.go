...
import (
	...
	"github.com/orderpulse/orderpulse/internal/correctness"
	"github.com/orderpulse/orderpulse/internal/models"
	"github.com/orderpulse/orderpulse/internal/queue"
	"github.com/orderpulse/orderpulse/internal/telemetry"
...
func (e *SandboxExecutor) Execute(ctx context.Context, j queue.Job) (*models.BenchmarkResults, error) {
	...
	defer func() {
		...
		if stopErr := e.docker.Stop(stopCtx, containerID); stopErr != nil {
			log.Warn("executor: failed to stop container on cleanup",
				"container_id", containerID, "err", stopErr)
		}
	}()
	...
	sub.ContainerName = fmt.Sprintf("orderpulse-%s", sub.ID[:8])
	...
	log.Info("executor: waiting for container to become healthy",
		"container_id", containerID, "host_port", hostPort)
...
func (e *SandboxExecutor) waitHealthy(ctx context.Context, containerID string, targetURL string, log *slog.Logger) error {
	...
	for {
		...
		log.Debug("executor: not yet healthy, retrying",
			"container_id", containerID, "poll_interval", e.healthPollInterval)
		select {
		case <-ctx.Done():
			return ctx.Err()
