...
const (
	...
	labelSubmissionID = "orderpulse.submission_id"
)
...
func (m *DockerManager) Deploy(ctx context.Context, sub *models.Submission) (containerID string, hostPort int, err error) {
	...
	containerName := fmt.Sprintf("orderpulse-%s", sub.ID[:8])
	...
	resp, err := m.cli.ContainerCreate(ctx,
		&container.Config{
			...
			Labels: map[string]string{
				...
				"orderpulse.team":     sub.TeamName,
				"orderpulse.language": string(sub.Language),
				"orderpulse.image":    image,
			},
		},
...
func (m *DockerManager) Stop(ctx context.Context, containerID string) error {
	...
	if err := m.cli.ContainerRemove(ctx, containerID, container.RemoveOptions{}); err != nil {
		slog.Warn("failed to remove container after stop", "id", containerID, "err", err)
	}
...
func buildEnv(sub *models.Submission) []string {
	return []string{
		"ORDERPULSE_SUBMISSION_ID=" + sub.ID,
		"ORDERPULSE_TEAM=" + sub.TeamName,
		"ORDERPULSE_LANGUAGE=" + string(sub.Language),
		"ORDERPULSE_PROTOCOL=" + string(sub.Protocol),
		"ORDERPULSE_LISTEN_PORT=7878",
	}
}
...
func (m *DockerManager) tailLogs(ctx context.Context, containerID, submissionID string) {
	...
	if err != nil {
		slog.Warn("could not attach container log stream", "container", containerID, "err", err)
		return
	}
	defer reader.Close()
